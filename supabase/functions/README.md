...
هذه الدوال تحمل مفتاح Gemini على الخادم.
...
يحوّل كلام المستخدم إلى نداء أداة أو ردّ نصّي، بـ function calling.

**لا ينفّذ شيئاً.** الدالّة تقترح فقط؛ التنقّل وكتابة الحركات تحدث على
الجهاز بعد تحقّق محلّي من الوسائط (`src/lib/assistantTools.ts`) وبعد
موافقة المستخدم على كل فعل يكتب. حتى لو انحرف الطراز — أو وجّهه نصّ
مدسوس داخل اسم جهة اتصال — فأقصى ما يبلغه اقتراحٌ مرئيّ يرفضه المستخدم.

الأدوات المعلنة: `navigateTo` و`createTransaction` و`toggleModal`.
أسماؤها ووجهاتها مرآةٌ لما يفكّه الجهاز، ويحرس التطابقَ اختبارٌ في
`tests/assistant.test.ts` فلا ينحرف أحد الطرفين صامتاً.
